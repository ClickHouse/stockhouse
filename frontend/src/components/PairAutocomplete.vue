  ...
  <div class="flex w-full justify-start items-center gap-2 px-1">
    <div class="relative w-[250px]">
      ...
      <Transition name="dropdown">
        <div
          v-show="showDropdown && filteredPairs.length > 0"
        ...
        >
          <button
            ...
            :class="isSelected(pair.pair) ? 'bg-neutral-700' : ''"
            @mousedown.prevent="togglePair(pair.pair)"
          >
            <div class="flex items-center gap-3">
              <span class="font-medium text-white">{{ pair.pair }}</span>
            </div>
            <div
              v-if="isSelected(pair.pair)"
    ...
    <button
      v-if="selectedPairs && selectedPairs.length > 0"
      type="button"
...
const emit = defineEmits(['toggle-pair', 'reset'])
...
const filteredPairs = computed(() => {
  ...
  if (!query) {
    // Show first 10 pairs when no search query
    return props.availablePairs.slice(0, 100)
  }
  
  // Filter by pair containing the query
  const filtered = props.availablePairs.filter(p => 
...
const isSelected = (pair) => {
  return props.selectedPairs && props.selectedPairs.includes(pair)
}

const togglePair = (pair) => {
  emit('toggle-pair', pair)
